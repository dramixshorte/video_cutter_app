...
<div class="card" style="max-width:560px;">
    <h3 style="margin-top:0;font-size:1.05rem;">تعديل المسلسل</h3>
    <form method="POST" action="{{ route('admin.series.update',$series) }}" class="grid" style="gap:1rem;">
        @csrf @method('PUT')
        <label>العنوان
            <input type="text" name="title" value="{{ old('title',$series->title) }}" required />
        </label>
        <label>الوصف
            <textarea name="description">{{ old('description',$series->description) }}</textarea>
        </label>
        <div style="display:flex;gap:1rem;">
            <label style="flex:1;">الترتيب
                <input type="number" name="order_index" value="{{ old('order_index',$series->order_index) }}" />
            </label>
            <label style="align-self:flex-end;margin-top:1.35rem;display:flex;flex-direction:row;align-items:center;gap:.4rem;font-weight:400;">
                <input type="checkbox" name="is_active" value="1" {{ $series->is_active ? 'checked':'' }} /> فعال
            </label>
        </div>
